...
class TestDatabaseHandler(unittest.TestCase):
    ...
    def test_create_database(self):
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='raw_automations'")
        self.assertIsNotNone(cursor.fetchone())
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='processed_automations'")
        self.assertIsNotNone(cursor.fetchone())
    ...
    def test_store_raw_data(self):
        ...
        self.db_handler.store_raw_data({'platform': 'state', 'condition': 'sun', 'service': 'light.turn_on'})
        ...
        self.assertEqual(rows[0][2], '{"platform": "state", "condition": "sun", "service": "light.turn_on"}')
    ...
    def test_store_data(self):
        ...
        self.db_handler.store_data(np.array(['state']), np.array(['sun']), np.array(['light.turn_on']))
